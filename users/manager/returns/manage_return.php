				...
				<div class="table-responsive" style="overflow-y: scroll;">
					<table class="table table-striped table-hover" id="tbl-cart-items" style="margin-bottom: 0;">
						...
						<tbody class="js-items">
							<?php
								$items = $conn->query("SELECT customer_name, customer_contact_number, customer_home_address, sti.product_id, p.name, sti.quantity, p.product_image, sti.session_id FROM `sales_transactions_2` st JOIN sales_transactions_2_items sti ON st.session_id = sti.session_id JOIN products p ON sti.product_id = p.id WHERE customer_name = '{$_GET['customer_name']}' AND transaction_number = '{$_GET['transaction_number']}' ");
                        ...
                        <td class="text-primary product_name">
                            ...
                            <div class="input-group my-3" style="max-width:150px">
                                ...
                                <input onblur="calculate_item_quantity('<?php echo $row['product_id'] ?>', `<?php echo $row['quantity'] ?>`)" onchange="calculate_item_quantity('<?php echo $row['product_id'] ?>', `<?php echo $row['quantity'] ?>`)" onkeyup="calculate_item_quantity('<?php echo $row['product_id'] ?>', `<?php echo $row['quantity'] ?>`)" type="text" data-product_id="<?php echo $row['product_id'] ?>" data-product_name="<?php echo $row['name'] ?>" data-product_quantity="<?php echo $row['quantity'] ?>" data-product_session_id="<?php echo $row['session_id'] ?>" class="form-control text-primary quantity<?php echo $row['product_id'] ?>" value="0" min="0" id="quantity" />
	...
	$(document).ready(function(){
		$('#submit-return').click(function(){
			...
			let productsData = checkIfHasQty.map((el, i) => {
              ...
              return ` ('${userFullName}', '${userContactNumber}', '${userHomeAddress}', ${el.product_id}, '${el.product_name}', ${el.product_quantity}, '${el.reason_for_return}', 1, '${el.product_session_id}')`;
            })
