<?php
    ...
    $sql_submit = "INSERT INTO `sales_transactions_2` (`customer_id`, `customer_name`, `customer_contact_number`, `customer_home_address`, `transaction_number`, `session_id`, `total_amount`, `amount_rendered`, `changed`, `discount_pct`, `vat_pct`, `delivery_fee`, `overall_total`, `status`) 
                        VALUES ('$customer_id', '$customer_name', '$customer_contact_number', '$customer_home_address', '$transaction_number', '$session_id', '$total_amount', '$amount_rendered', '$changed', '$discount_pct', '$vat_pct', '$delivery_fee', '$overall_total', '$status') 
    ...
    if(mysqli_query($conn,$sql_submit) && mysqli_query($conn,$sql_move_cart_items)) {
        ...
        if(mysqli_num_rows($result) > 0)
        {
            while($row = mysqli_fetch_array($result))
            {
                ...
                $sql_product = "SELECT * FROM products WHERE id='$product_id' LIMIT 1";
